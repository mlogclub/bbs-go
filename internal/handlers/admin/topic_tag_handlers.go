...
func TopicTagDetail(ctx *gin.Context) {
...
func TopicTagList(ctx *gin.Context) {
	list, paging := services.TopicTagService.FindPageByParams(params.NewQueryParams(ctx).PageByReq().Desc("id"))
	ginx.WriteJSON(ctx, &web.PageResult{Results: list, Page: paging})

}

func TopicTagCreate(ctx *gin.Context) {
	t := &models.TopicTag{}
...
func TopicTagUpdate(ctx *gin.Context) {
