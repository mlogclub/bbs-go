...
func InitIris() {
	...
	server := &http.Server{Addr: ":" + utils.Conf.Port}
	handleSignal(server)
...
func handleViews(app *iris.Application) {
	app.StaticWeb("/static", utils.Conf.StaticPath)
	app.StaticWeb("/", utils.Conf.StaticPath)

	engine := iris.HTML(utils.Conf.ViewsPath, ".html").Reload(true)
...
func viewFunctions(engine *view.HTMLEngine) {
	engine.AddFunc("siteTitle", func(siteTitle string) string {
		if len(siteTitle) > 0 {
			return siteTitle + " | " + utils.Conf.SiteTitle
		}
		return utils.Conf.SiteTitle
	})
