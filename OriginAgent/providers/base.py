...
class LLMProvider(ABC):
    ...
    def _sanitize_empty_content(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        messages = LLMProvider.sanitize_for_provider(messages)
    ...
    def _sanitize_request_messages(
        ...
        sanitized = []
        for msg in LLMProvider.sanitize_for_provider(messages):
    ...
    @staticmethod
    def sanitize_for_provider(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Return a provider-facing copy with internal metadata removed.

        The agent loop keeps ``_meta.kind`` on content blocks so persistence can
        decide what to keep. Provider adapters must not see that metadata, and
        this helper must never mutate the caller's message list or block dicts.
        """
