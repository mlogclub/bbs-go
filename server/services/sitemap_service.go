...
func (s *sitemapService) Generate(dateFrom, dateTo int64) {
	...
	sm := sitemap.NewGenerator(config.Conf.Uploader.Minio.Host, sitemapPath, sitemapName, func(sm *sitemap.Generator, sitemapLoc string) {
...
func (s *sitemapService) GenerateMisc() {
	sm := sitemap.NewGenerator(config.Conf.Uploader.Minio.Host, sitemapPath, "sitemap-misc",
...
func (s *sitemapService) GenerateUser() {
	sm := sitemap.NewGenerator(config.Conf.Uploader.Minio.Host, sitemapPath, "sitemap-user",
		func(sm *sitemap.Generator, sitemapLoc string) {
			s.AddSitemapIndex(sm, sitemapLoc)
		})
